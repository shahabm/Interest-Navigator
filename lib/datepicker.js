//Modified by MANTEC CORP. Hooks were added to give customization ability.
//Other than the loading method, the interface is unchanged.
//
//Unmodified original can be found at: http://chrishulbert.github.io/datepicker/
//------------------------------------------------------------------------------
//
...
function chooseDate(e) {
  ...
  textbox.value = getDateString( targ.getAttribute( 'date' )); // Set the selected date
...
function createCalendar(div, month ) {
  ...
  while (1) { // Loop for each week
    ...
    for (i=0;i<7;i++) { // Loop each day of this week
      ...
      inp.setAttribute( 'date', date.valueOf() );
...
function showDatePicker( idOfTextbox, picker_fn ){
  ...
  for (i=0;i<x.length;i++) {
    if (x[i].classList.contains('datepickerdropdown')) {
  ...
  div.classList.add( 'datepickerdropdown' );
...
// icon_fn: optional callback for after the picker icon is created
// picker_fn: optional callback for after the picker icon is clicked
// format_fn: optional to format the date with.
function attachPicker( icon_fn, picker_fn, format_fn ){
  var dispatchShowCalendar = function( id ){
    return function(){ showDatePicker( id, picker_fn );}
  };

  var allPickers = document.getElementsByClassName("datepicker");

  for (i=0; i<allPickers.length; i++) {
    ...
    if( picker_fn )
      a.onclick = dispatchShowCalendar( allPickers[i].id );
    ...
    insertAfter( a, picker );

    if( icon_fn )
      icon_fn( picker, a );
  }
  if( format_fn )
    getDateString = format_fn;
}
